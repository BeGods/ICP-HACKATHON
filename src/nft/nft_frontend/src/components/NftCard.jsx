...
const NftCard = ({
  ...
  const transferNFT = async () => {
    // img.tokenId, img.collectionId
    console.log("yes transfer was called");
  ...
  return (
    ...
    >
      <div
        className={`flip-card-inner  ${!img.isOwned && "border-3"} ${
        ...
        <div className="relative flex flex-col items-center justify-center text-white flip-card-back ">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black rounded-lg bg-opacity-60">
            ...
            <h2 className="mt-2 text-lg sm:text-xl">
              Type : {img.cardType.charAt(0).toUpperCase()}
              {img.cardType.slice(1)}
            </h2>
            ...
            {!img.isOwned && (
              <h2 className="mt-1 text-lg sm:text-xl">{img.price} ICP</h2>
            )}
            {img.isOwned ? (
              showLoader ? (
              ...
              <Link
                to={`/Nft/${img.name}/buy?collectionId=${img.collectionId}&type=${img.collectionColor}&index=${img.index}`}
            ...
            {img.isOwned && (
              <Link
                to={`/Nft/${img.cardName}/buy?collectionId=${img.collectionId}&type=${img.collectionColor}&index=${img.index}`}
                className="flex items-center justify-center mt-4 w-[60%] h-[30px] sm:w-[150px] sm:h-[32px] bg-blue-400 text-black border-3px border-gray-100 shadow-lg transform transition-transform hover:scale-105 font-caslon"
              >
                View Details
              </Link>
